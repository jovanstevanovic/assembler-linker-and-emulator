...
SIZE SymbolTable::getSectionSize(SECTION section) {
...
SER_NUM SymbolTable::getSerialNumber(SER_NUM sn, SymbolTable* globalSymbolTable) {
	for (TableRow *cur = tablePointer; cur != 0; cur = cur->next) {
		if (cur->sn == sn) {
			return globalSymbolTable->getSerialNumber(cur->name);
		}
	}
	return UNDEFINED_VALUE;
}

SECTION SymbolTable::getSectionBySerialNumber(SER_NUM sn) {
	for (TableRow *cur = tablePointer; cur != 0; cur = cur->next) {
		if (cur->sn == sn) {
			return cur->sec_num;
		}
...
long SymbolTable::getObjFileSize() {
	return allSectionsSize;
}
...
SIZE SymbolTable::updateSymbolTable(SECTION section, long locCounter, SIZE sectionsSize) {
	SIZE curSectionSize = getSectionSize(section);
	if (curSectionSize == UNDEFINED_VALUE) // There is no section with given name in this obj file.
		return 0;
	
	for (TableRow* curRow = symbolTHead; curRow != nullptr; curRow = curRow->next) {
		if (curRow->sec_num == section && curRow->vis == GLOBAL) 
			curRow->value = curRow->value + locCounter + sectionsSize;
	}
	return curSectionSize;
}
